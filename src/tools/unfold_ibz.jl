...
function write_full_data(seedname)
    ...
    Rn = find_pos(isym.symops, nnkp.projections, isym.D)
    println(Rn)
    ...
    Mfull, kpb_kfull, kpb_Gfull = unfold_mmn(
        M, kpb_k, kpb_G, nnkp, isym, f2i, bvectors_cryst
    )
...
function unfold_mmn(M, kpb_k, kpb_G, nnkp, isym, f2i, bvectors_cryst)
    ...
    Mfull = [
        [zeros(ComplexF64, size(M[1][1])) for _ in 1:size(M[1])[1]] for
        _ in 1:size(nnkp.kpoints)[1]
    ]
    kpb_kfull = [[0 for _ in 1:size(M[1])[1]] for _ in 1:size(nnkp.kpoints)[1]]
    kpb_Gfull = [
        [zeros(Float64, 3) for _ in 1:size(M[1])[1]] for _ in 1:size(nnkp.kpoints)[1]
    ]
    ...
    for ikf in 1:size(nnkp.kpoints)[1]
        ...
        for ibf in 1:size(bvectors_cryst)[1]
            ...
            isym_h, tDelta = get_symop(
                isym.symops, [[isym_ikbi, -1], [isym_ikf, -1], [isym_ikbf, +1]]
            )
            ...
            Mfull[ikf][ibf] =
                Mfull[ikf][ibf]*exp(
                    im*2.0*π*(dot(bi, isym.symops[isym_ikf].ft) + dot(kbi_ibz, tDelta))
                )
        end
    end
