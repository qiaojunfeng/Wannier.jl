...
function symmetrize(Op, isym, Rn)
    ...
    for ik in 1:nk_ibz
        for ih in 1:size(isym.gk[ik].h)[1]
            ...
            phase = [
                exp(-im*2.0*π*dot(isym.kpoints[ik], Rn[ih_index][iwann][:])) for
                iwann in 1:n_wann
            ]
            Op_h = apply_sym_Uk(
                Op[ik],
...
function unfold_amn(A, f2i, Rn, isym)
    ...
    for ik in 1:num_kpoints
        ...
        phase = [
            exp(-im*2.0*π*dot(isym.kpoints[ik_ibz], Rn[sym_index][iwann][:])) for
            iwann in 1:n_wann
        ]
        Afull[ik] = apply_sym_Uk(
            A[ik_ibz], phase, isym.D[sym_index], isym.symops[sym_index].t_rev
        )
    end
    return Afull
end
...
function find_pos(symops, proj, D)
    ...
    for isym in 1:n_symm
        for iwann in 1:n_wann
            ...
            #println(iRw)
            @assert all(
                x -> x == proj[iRw[1]].center,
                [proj[iRw[iwann_]].center for iwann_ in 1:size(iRw)[1]],
            )
...
function get_symop(symops, symlist)
    ...
    for isym in 1:size(symops)[1]
        symop = symops[isym]
        tDelta = ft0 - symop.ft
        if isapprox(s0, symop.s; atol=1.e-06) &&
            isapprox(tDelta, round.(tDelta); atol=1.e-06) &&
            (t_rev == symop.t_rev)
            return isym, tDelta
        end
    end
