...
function TBHamiltonian(model::Model; kwargs...)
    return TBHamiltonian(
        model, get_Rvectors_mdrs(model.lattice, model.kgrid, center(model)); kwargs...
    )
end

function TBHamiltonian(model::MagModel, args...; kwargs...)
    uphami = TBHamiltonian(model.up, args...; kwargs...)
    dnhami = TBHamiltonian(model.dn, args...; kwargs...)

    map(zip(uphami, dnhami)) do (up_b, dn_b)
    
        block = ColinMatrix(up_b.block, dn_b.block)
        tb_block = ColinMatrix(up_b.tb_block, dn_b.tb_block)
        
        return TBBlock(up_b.R_cryst, up_b.R_cart, block, tb_block)
    end
end
