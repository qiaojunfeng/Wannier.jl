...
function interpfft(A, newSize)
    ...
    Y_padded[(div(newSize - N1, 2) + 1):div(newSize + N1, 2), (div(newSize - N2, 2) + 1):div(newSize + N2, 2), (div(newSize - N3, 2) + 1):div(newSize + N3, 2)] = fftshift(
        Y
    )
...
function interpFourier(A)
    ...
    function interpA(x)
        ...
        for k in 1:size(A)[3], j in 1:size(A)[2], i in 1:size(A)[1]
            poly +=
                Y[i, j, k] *
                exp(im * 2 * pi * dot([(i - 1); (j - 1); (k - 1)] .- half_size, x))
        end
