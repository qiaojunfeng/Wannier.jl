...
function transform_gauge(
    ...
    (size(U[1], 1), length(U)) == (nbands, n_kpts) ||
        error("U must have size (nbands, :, n_kpts)")
    # The new n_wann
    n_wann = size(U[1], 2)

    # the new gauge is just identity
    U2 = identity_gauge(eltype(U[1]), n_kpts, n_wann)

    # EIG
    E = model.eigenvalues
    E2 = map(m -> similar(m), E)
    H = zeros(eltype(model.gauges[1]), n_wann, n_wann)
