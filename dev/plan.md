...
- Removal of the `_build_fg_*` / `LocalizationProblem(:symbol)` plumbing. ✅ Resolved.
- `_make_optim_fg!(prob)` in [src/localization/solver.jl](src/localization/solver.jl) now produces the Optim-compatible `fg!` directly from `(Objective, Model, Layout)`. `solve!` dispatches there; `LocalizationProblem{Symbol}` / `build_fg!` / `_build_fg_maxloc` / `_build_fg_disentangle` / `_build_fg_mag_disentangle` / `_build_fg_mag_disentangle_center` / `_accumulate_*` / `AbstractLocalizationTerm` / `VarianceTerm` / `CenterConstraintTerm` / `_terms_to_objective` / `disentangle_bands` / `localize_isolated_bands` / `max_localize(terms, ...)` / `disentangle(terms, ...)` / `disentangle(sm, λ; ...)` / `get_fg!_maxloc` / `get_fg!_disentangle` / `omega(terms, ...)` are all deleted. Tests now call `_make_optim_fg!(Problem(Variance/CenteredVariance/CoOptVariance/CenteredCoOptVariance, model))` directly and use `coopt` / `constrain_center_coopt` / `localize` as the only entry points.
...
**Commit CC (optional)** — add `ManoptLBFGS <: AbstractLocalizationSolver`. Implement `solve!(prob, ::ManoptLBFGS)` using `Manopt.jl` + `Manifolds.jl`. `manifold(layout, model, ::ManoptLBFGS)` returns `Manifolds.Stiefel` variants. This is strictly additive — `OptimLBFGS` keeps working. Benchmark against `OptimLBFGS`; decision to promote `ManoptLBFGS` as default is a later call, not a Phase 7 deliverable. ✅ Partially done (Variance + UGauge branch).
- `ManoptLBFGS <: AbstractLocalizationSolver` lives in [src/localization/solver.jl](src/localization/solver.jl) alongside `OptimLBFGS`; the fallback `solve!(::Problem, ::ManoptLBFGS)` error points users at either `OptimLBFGS` or loading Manopt.
- Backend-specific implementation is a **package extension**: `Manopt` / `Manifolds` declared in `Project.toml` `[weakdeps]` + `[extensions] WannierManoptExt`, code in [ext/WannierManoptExt/WannierManoptExt.jl](ext/WannierManoptExt/WannierManoptExt.jl). Main-package load has no Manopt dependency; `using Manopt, Manifolds` activates the extension.
- Extension implements `solve!(Problem{<:Variance, <:Model, <:UGauge}, ::ManoptLBFGS)` — PowerManifold(Stiefel(nb, nw, ℂ), NestedPowerRepresentation(), nk), cost + Riemannian gradient via `project(St, p, ∇f)`, `quasi_Newton(...; memory_size, stopping_criterion)`. Reuses the main-package `_make_optim_fg!` closure + `Workspace` so there is a single gradient path under both backends.
- Tests (`test/localization/manopt.jl`) assert the UGauge gauge agrees with `OptimLBFGS` to `atol = 1e-6` and that the entangled XYGauge path still errors with the not-yet-implemented message.
- Deferred: XYGauge / ProductLayout / WLayout bindings. `manifold(layout, model, ::ManoptLBFGS)` as a trait is not yet needed — the current UGauge path constructs `PowerManifold(Stiefel(...), ...)` inline; that factoring can land alongside the XYGauge/Product variants.
